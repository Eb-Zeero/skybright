...
from bokeh_server.none_plots import functions as fn
...
'''
date_ = fn.selector_to_date(year_.value, month_.value, day_.value)

data = fn.read_database(date_, 1)
ext_data = fn.read_extinction_database(date_, 1)
range_data = fn.read_range_database([range_year_min.value, range_month_min.value, range_day_min.value],
        ...
        for p in range(5):
            for f in range(4):
                ...
                if len(med_days[t][p][f]) > 0 and len(med_list[t][p][f]) > 0:
                    ...
                    range_span_source[t][p][f].data['h_date'] = fn.append_twice("Median Line Statistics")

                    range_span_source[t][p][f].data['y'] = fn.append_twice(np.median(med_list[t][p][f]))
                    range_span_source[t][p][f].data['error'] = fn.append_twice(np.average(med_err[t][p][f]))
                    range_span_source[t][p][f].data['coverage'] = fn.append_twice('Average: ' +
                                                                                  str(np.average(med_list[t][p][f])))
                    range_span_source[t][p][f].data['telescope'] = fn.append_twice(
                        'Min: ' + str(min(med_list[t][p][f])))
                    range_span_source[t][p][f].data['filter'] = fn.append_twice('Max: ' + str(max(med_list[t][p][f])))
                    range_span_source[t][p][f].data['position'] = fn.append_twice(med_pos[t][p][f][0])
                else:
    ...
    for t in range(2):
        for p in range(5):
            for f in range(4):
                ...
                shd = fn.append_twice("Median line statistics") if len(temp_list) > 0 else []
                smed = fn.append_twice(np.median(temp_list)) if len(temp_list) > 0 else []
                savg = fn.append_twice(np.average(temp_err)) if len(temp_list) > 0 else []
                scov = fn.append_twice('Avg: ' + str(np.average(temp_list))) if len(temp_list) > 0 else []
                smin = fn.append_twice('Min: ' + str(min(temp_list))) if len(temp_list) > 0 else []
                smax = fn.append_twice('Max: ' + str(max(temp_list))) if len(temp_list) > 0 else []
                ...
                for f in range_checkbox_list[2]:
                    ...
                    if f != 4:
                        ...
                        if len(l) > 0:
                            ...
                            temp_tel[t][p][f].append(fn.find_telescope_name(t))
                            temp_pos[t][p][f].append(fn.find_position(p))
                            temp_fil[t][p][f].append(fn.find_filter_name(f))
...
def week_button_clicked():
    ...
    d_label.set(text='Loading . ')
    date_ = fn.selector_to_date(year_.value, month_.value, day_.value)
    data = fn.read_database(date_, 7)
    d_label.set(text='Loading . . ')
    ext_data = fn.read_extinction_database(date_, 7)
    d_label.set(text='Loading . . . ')
    create_list()
    update_data_source()
    d_label.set(text='Loading . ')
...
def range_button_clicked():
    ...
    if fn.selector_to_date(range_year_min.value, range_month_min.value, range_day_min.value) > \
        fn.selector_to_date(range_year_max.value, range_month_max.value, range_day_max.value):
        range_data = []
        create_range_list()
        r_label.set(text='Loading . . ')
...
def create_and_set_annotations(min_day):
    ...
    for day in moon_up_date:
        for i in range(6):
            r_and_s = fn.find_moon_rise_set(str(day))
...
def create_list():
    ...
    '''
    date_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                     [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])

    h_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                  [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])

    data_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                     [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    error_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                      [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    coverage_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                         [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    cc_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                   [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    moon_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                     [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    telescope_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                          [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    filter_list = fn.append_twice([[[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]],
                       [[[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []], [[], [], [], []]]])
    '''
    ...
    for d in data:
        ...
        telescope_list[t][p][f].append(fn.find_telescope_name(t))
...
def create_range_list():
    ...
    for d in range_data:
        ...
        date_ = fn.find_range_date(d[0])

        if date_ not in range_date[t][p][f]:
            range_date[t][p][f].append(date_)
            add_day_list()

        nd = fn.find_date_index(date_, range_date[t][p][f])
...
for t in range(2):
    ...
    for p in range(5):
        for f in range(4):
            ...
            col_ = fn.set_colour_range(t, p, f)
            if t == 0:
